...
export default apiInitializer("1.0", (api) => {
  ...
  api.decorateCookedElement(
    async (el, helper) => {
      ...
      // ── gravity blocks on vault items: the relationship sections ──
      if (false) el.querySelectorAll('.d-wrap[data-wrap="curio-gravity"]').forEach((w) => {
        if (w.querySelector(".curio-gravhead")) return;
        const slug = decode(w.dataset.concept || "");
        const head = document.createElement("div");
        head.className = "curio-gravhead";
        {
          const ex = parseFloat(w.dataset.ex), ey = parseFloat(w.dataset.ey);
          const sfx = slug.replace(/-/g, "_");
          const gx = pollMean((post?.polls || []).find((x) => x.name === `axis_x_${sfx}`));
          const gyy = pollMean((post?.polls || []).find((x) => x.name === `axis_y_${sfx}`));
          const hc = gx && gyy && Math.min(gx.voters, gyy.voters) >= settings.min_voters;
          const g = document.createElement("span");
          g.className = "cg-glyphholder";
          g.innerHTML = miniGrid(ex, ey, hc ? gx.mean : null, hc ? gyy.mean : null, w.dataset.tier || "", 44);
          head.append(g);
        }
        const name = document.createElement("a");
        name.className = "cg-concept";
        name.href = `/tags/c/curiobase/${settings.curiobase_id}/${slug}`;
        name.textContent = prettify(slug);
        head.append(name);
        if (w.dataset.tier) {
          const t = document.createElement("span");
          t.className = "cw-tier";
          t.style.borderColor = tierColor(w.dataset.tier);
          t.style.color = tierColor(w.dataset.tier);
          t.textContent = decode(w.dataset.tier).toUpperCase();
          head.append(t);
        }
        const ed = document.createElement("span");
        ed.className = "cw-score";
        ed.textContent = `${settings.label_editorial}: ${w.dataset.ex || "—"} · ${w.dataset.ey || "—"}`;
        head.append(ed);
        const suffix = slug.replace(/-/g, "_");
        const px = pollMean((post?.polls || []).find((x) => x.name === `axis_x_${suffix}`));
        const py = pollMean((post?.polls || []).find((x) => x.name === `axis_y_${suffix}`));
        const cm = document.createElement("span");
        cm.className = "cw-score cw-community";
        if (px && py && Math.min(px.voters, py.voters) >= settings.min_voters) {
          cm.textContent = `${settings.label_community}: ${px.mean.toFixed(1)} · ${py.mean.toFixed(1)} (${Math.min(px.voters, py.voters)})`;
        } else {
          cm.textContent = `${settings.label_community}: ${settings.label_awaiting}`;
          cm.classList.add("cw-awaiting");
        }
        head.append(cm);
        w.prepend(head);
        // echo the concept's opening definition beneath the header (cached fetch)
        (async () => {
          try {
            if (w.querySelector(".cg-def")) return;
            const feed = await fetchJson(`/tags/c/curiobase/${settings.curiobase_id}/${slug}.json`);
            const ct = (feed.topic_list?.topics || []).find((t) => t.category_id !== settings.nominations_id);
            if (!ct) return;
            const cj = await fetchJson(`/t/${ct.id}.json`);
            const h = document.createElement("div");
            h.innerHTML = cj.post_stream.posts[0].cooked;
            const nameLink = w.querySelector(".cg-concept");
            if (nameLink) nameLink.href = `/t/${ct.slug}/${ct.id}`;
            const p1 = [...h.querySelectorAll("p")].find((p) => p.textContent.trim().length > 40);
            if (!p1) return;
            const def = document.createElement("div");
            def.className = "cg-def";
            let txt = p1.textContent.trim().replace(/\s+/g, " ");
